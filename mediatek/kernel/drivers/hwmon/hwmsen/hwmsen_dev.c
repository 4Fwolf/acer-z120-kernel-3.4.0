...
#define MAX_CHOOSE_ALSPS_NUM 5  //maliyan
...
#endif
#if defined(MTK_AUTO_DETECT_ALSPS)//maliyan
static char alsps_sensor_name[25];
static struct sensor_init_info* alsps_sensor_init_list[MAX_CHOOSE_ALSPS_NUM]= {0}; //modified
#endif
...
#if defined(ACER_Z2) || defined(ACER_Z3)//LK@add fix gsensor/psensor/TP work abnormal issue
       hwPowerOn(MT65XX_POWER_LDO_VIO28, VOL_2800, "KXTIK1004");
       msleep(10);
       hwPowerOn(MT65XX_POWER_LDO_VGP2, VOL_2800, "stk3171");
       msleep(10);
...
/*
     for(i = 0; i < MAX_CHOOSE_G_NUM; i++)
     {
       HWM_LOG(" alsps_sensor_init_list[i]=%d\n",alsps_sensor_init_list[i]);
     }
*/
	//
	for(i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++)
	...
	{
	   HWM_LOG(" alsps_sensor probe fail\n");
	}
	return 0;
}
