...
static int charger_low_counter=0; //LK@add
///////////////////////////////////////////////////////////////////////////////////////////
...
static unsigned char chrind_led_status = 0;
...
typedef struct 
{
    ...
    UINT32       ADC_I_SENSE;
    UINT32   	bat_temperature_status;  /*yanfang 20120307 modify for battery temprature check*/
#if defined(ACER_Z1) || defined(ACER_Z2) //for Acer charger test
...
#if defined(ACER_Z2)
int CHARGING_FULL_CURRENT=190;    // mA 
#else
...
#if defined(ACER_Z1) || defined(ACER_Z2)//for Acer charger test
    static int  g_BAT_ChargerTest = 0;
#endif  
...
#if defined(ACER_Z1) || defined(ACER_Z2)  //for C8 charger test
static int mt6329_battery_set_property(struct power_supply *psy,
	enum power_supply_property psp,
	union power_supply_propval *val)
{
    int ret = 0;     
...
static int  mt6329_battery_property_is_writeable(struct power_supply *psy,enum power_supply_property psp)
{
    int ret = 0;     
...
static struct mt6329_battery_data mt6329_battery_main = {
  ...
  #if defined(ACER_Z1) || defined(ACER_Z2)  //for ACER charger test
    .set_property = mt6329_battery_set_property,  
    .property_is_writeable = mt6329_battery_property_is_writeable, 
  #endif
    },
...
static void mt6329_battery_update(struct mt6329_battery_data *bat_data)
{
    ...
    {     
        if ( BMT_status.bat_exist )                
        {
...
#else		                
            if ( (BMT_status.bat_vol >= RECHARGING_VOLTAGE) && (BMT_status.bat_full == KAL_TRUE) )//LK@:charging current < CHARGING_FULL_CURRENT
#endif
            {
                /*Use no gas gauge*/
                ...
                {                    
                    ...
                    if(bat_volt_check_point>=100)
                    {
                        ...
                        //LK@++
                        #if defined(ACER_Z2)
                        if(1 == chrind_led_status)
                        {
                            upmu_chr_chrind_on(0);
                            chrind_led_status = 0;
                            printk("Power/Battery LK+++[mt6329_battery_update]:turn off LED");
                        }
                       #endif
                    }
            ...
            /* battery charging */
            else //LK@:SOC get 100% first, wait charging current < CHARGING_FULL_CURRENT
            {
...
#elif (V_CHARGER_COMPATIBLE_ENABLE == 1)
            if (g_is_500mA_charger == 1)
            {
                g_temp_CC_value = AC_CHARGER_COMPATIBLE_CURRENT;
			}
			else
			{
            #if defined(ACER_Z1) || defined(ACER_Z2)
			    if (BMT_status.temperature >= 40)
			    {
...
#if defined(ACER_Z1) || defined(ACER_Z2)
    if((g_bat_full_user_view == KAL_FALSE) && /*battery display is not full, not recharging mode*/
        (bat_volt_check_point < 100))//battery capacity < 100%
    {
        if(0 == chrind_led_status)
        {
            upmu_chr_chrind_on(1);
            chrind_led_status = 1;
        }
    }
#endif
...
#if defined(ACER_Z1) || defined(ACER_Z2)
    if(((0 == g_HW_stop_charging) && (0 == g_Charging_Over_Time)) ||/*CC mode get voltage, stop charging but not to turn off led;bypass charging OT, use another handle*/
        (g_bat_full_user_view == KAL_TRUE)//recharging, keep full state,turn off led
        )
    {
        if(1 == chrind_led_status)
        {
            upmu_chr_chrind_on(0);
            chrind_led_status = 0;
            printk("Power/Battery LK+++[pchr_turn_off_charging]:turn off LED");
        }
    }
#endif	
}
...
void pchr_turn_on_charging (void)
{
    ...
    {          
        printk("chailu charge state=%d,charger_type=%d \n",BMT_status.bat_charging_state,BMT_status.charger_type); 
        printk("chailu charger_vol =%d \n",BMT_status.charger_vol);
        if (BMT_status.charger_vol <= V_CHARGER_COMPATIBLE_V )
        {
            charger_compatible_500mA_counter++;			
            printk("chailu vol small 500ma cnt = %d,\n",charger_compatible_500mA_counter);
...
int BAT_CheckBatteryStatus(void)
{
    ...
    if( do_jeita_state_machine() == PMU_STATUS_FAIL)
    {
	...
	#if (BAT_TEMP_PROTECT_ENABLE == 1)//LK@modify
		...
		else if (BMT_status.temperature <= MIN_CHARGE_TEMPERATURE) 
		{
			...
			BMT_status.bat_temperature_status=BATTERY_UNDER_TEMP;   /*yanfang 20120307 modify for battery temprature check*/
		#if defined(ACER_Z1) || defined(ACER_Z2)
        if(BMT_status.temperature <= MIN_CHARGE_NOTIFY_TEMPERATURE)//LK@add, notify MMI not to keep charging icon if temp under -10
            BMT_status.bat_charging_state = CHR_ERROR;
		#else
			BMT_status.bat_charging_state = CHR_ERROR;
		#endif
		...
		{
			if (BMT_status.bat_temperature_status==BATTERY_UNDER_TEMP)  /*yanfang 20120307 modify for battery temprature check*/
			{                
				BMT_status.bat_charging_state = CHR_PRE;
    ...
    if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
    {
		...
		BMT_status.bat_temperature_status=BATTERY_OVER_TEMP;    /*yanfang 20120307 modify for battery temprature check*/
	#if defined(ACER_Z1) || defined(ACER_Z2)
        if(BMT_status.temperature >= MAX_CHARGE_NOTIFY_TEMPERATURE)//LK@add, notify MMI not to kerp charging icon if temp over 55
            BMT_status.bat_charging_state = CHR_ERROR;
	#else
        BMT_status.bat_charging_state = CHR_ERROR;
	#endif
	...
	else//LK@ for recovery charging
	{
		if (BMT_status.bat_temperature_status==BATTERY_OVER_TEMP) /*yanfang 20120307 modify for battery temprature check*/
		{                
			BMT_status.bat_charging_state = CHR_PRE;
			BMT_status.bat_temperature_status=0;
		}
    ...
    if( upmu_is_chr_det() == KAL_TRUE)
    {
        ...
        if (BMT_status.charger_vol <= V_CHARGER_MIN )
        {
			...
			if(charger_low_counter>4)
			{
				...
				BMT_status.charger_protect_status =charger_UNDER_VOL;  /*yanfang 20120307 modify for charger check*/
				BMT_status.bat_charging_state = CHR_ERROR;
				charger_low_counter=0;
				return PMU_STATUS_FAIL;   
			}
			else if(BMT_status.charger_vol > V_CHARGER_MIN )//LK@ for recovery charging
			{
				if (BMT_status.charger_protect_status == charger_UNDER_VOL)  /*yanfang 20120307 modify for charger check*/
				{                  
        ...
        else//LK@add for recharging when condition recovered
        {
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
    ...
    if ( BMT_status.bat_vol > V_CC2TOPOFF_THRES )
    {
        ...
        {
            for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {                
                batteryCurrentBuffer[i] = AC_CHARGER_CURRENT; //LK@650->AC_CHARGER_CURRENT
            }
            batteryCurrentSum = AC_CHARGER_CURRENT * BATTERY_AVERAGE_SIZE;
        }
	 #if (V_CHARGER_COMPATIBLE_ENABLE == 1)
        else if(g_temp_CC_value == AC_CHARGER_COMPATIBLE_CURRENT)
		{
			for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {	            
				batteryCurrentBuffer[i] = AC_CHARGER_COMPATIBLE_CURRENT;
        	}
			batteryCurrentSum = AC_CHARGER_COMPATIBLE_CURRENT * BATTERY_AVERAGE_SIZE;
		}
	 #endif
        else
        {
            for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {                
                batteryCurrentBuffer[i] = USB_CHARGER_CURRENT; //LK@450->USB_CHARGER_CURRENT
            }
            batteryCurrentSum = USB_CHARGER_CURRENT * BATTERY_AVERAGE_SIZE;
        }                
    }
...
void mt_battery_notify_check(void)
{
    ...
    #if defined(ACER_Z1) || defined(ACER_Z2)//battery capcity is 1300mA, no larger than 1300mA
...
#if defined(ACER_Z1) || defined(ACER_Z2)
		if(BMT_status.temperature <= MIN_CHARGE_NOTIFY_TEMPERATURE)
#else
	...
	else if(g_BN_TestMode == 0x0006)
	{
		g_BatteryNotifyCode = 0x0020;
		xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY_TestMode] BATTERY_NOTIFY_CASE_0006\n");
	}
    else
    {
...
void BAT_thread(void)
{
    ...
    {   
        ...
        for (i=0; i<BATTERY_AVERAGE_SIZE; i++) {
           ...
           batteryCurrentSum = 0;
	  #if defined(ACER_Z1) || defined(ACER_Z2) //for C8 charger test
        //g_BAT_ChargerTest = 0;//LK@mask,run in will close this vulue when exit
      #endif
	 #if (V_CHARGER_COMPATIBLE_ENABLE == 1)
      charger_compatible_500mA_counter = 0; 
