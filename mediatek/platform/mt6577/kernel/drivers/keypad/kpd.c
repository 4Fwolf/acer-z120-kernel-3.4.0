...
/* for lowpower eint */
#if KPD_HAS_LOWPOWER_EINT
static void kpd_lowpower_handler(unsigned long data);
static DECLARE_TASKLET(kpd_lowpower_tasklet, kpd_lowpower_handler, 0);
static u8 kpd_lowpower_state = !KPD_LOWPOWER_POLARITY;
static u8 lowpower_key_state=0;//default:0--off
#endif
...
{
	bool lowpower;
	u8 old_state = kpd_lowpower_state;

	kpd_lowpower_state = !kpd_lowpower_state;
	lowpower = (kpd_lowpower_state == !!KPD_LOWPOWER_POLARITY);
	/* for SW_LOWPOWER, 1: enable => lowpower, 0: disable => normal */
	//input_report_switch(kpd_input_dev, SW_LOWPOWER, lowpower);
	//input_sync(kpd_input_dev);
	if(lowpower)
	{
		kpd_print("[LOWPOWER]report lowpower QWERTY ,state=on\n");
		input_report_key(kpd_input_dev, KEY_LOWPOWER_ON, 1);       
		input_sync(kpd_input_dev);
		input_report_key(kpd_input_dev, KEY_LOWPOWER_ON, 0);
		input_sync(kpd_input_dev);
		lowpower_key_state=1;
	}
	else
	{
		kpd_print("[LOWPOWER]report lowpower QWERTY ,state=off\n");
		input_report_key(kpd_input_dev, KEY_LOWPOWER_OFF, 1);       
		input_sync(kpd_input_dev);
		input_report_key(kpd_input_dev, KEY_LOWPOWER_OFF, 0);
		input_sync(kpd_input_dev);
		lowpower_key_state=0;
	}

	//if(old_state) {
	//	mt_set_gpio_pull_select(GPIO_QWERTYLOWPOWER_EINT_PIN, 0);
	//} else {
	//	mt_set_gpio_pull_select(GPIO_QWERTYLOWPOWER_EINT_PIN, 1);
	//}
	/* for detecting the return to old_state */
	mt65xx_eint_set_polarity(KPD_LOWPOWER_EINT, old_state);
	mt65xx_eint_unmask(KPD_LOWPOWER_EINT);
}

static void kpd_lowpower_eint_handler(void)
{
	tasklet_schedule(&kpd_lowpower_tasklet);
}


static struct proc_dir_entry *lowpower_proc_entry=NULL;
static int lowpower_key_state_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
/*
	memcpy(buf, &lowpower_key_state, 1);
	*eof = 1;
	*start = NULL;
	return 1;
*/
...
int init_proc_lowpower_key_state(void)
{
	int ret=0;
	lowpower_proc_entry = create_proc_entry( "lowpower_key_state", 0444, NULL );
	
	if (lowpower_proc_entry == NULL) {
		ret = -ENOMEM;
	  	kpd_print("[LOWPOWER]init_proc_lowpower_key_state: Couldn't create proc entry\n");
	} else {
		lowpower_proc_entry->read_proc = lowpower_key_state_read;
		lowpower_proc_entry->write_proc = NULL;//lowpower_key_state_write;
		kpd_print("[LOWPOWER]init_proc_lowpower_key_state loaded.\n");
	}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
...
#if KPD_HAS_LOWPOWER_EINT
	//__set_bit(EV_SW, kpd_input_dev->evbit);
	//__set_bit(SW_LOWPOWER, kpd_input_dev->swbit);
	__set_bit(KEY_LOWPOWER_ON, kpd_input_dev->keybit);
	__set_bit(KEY_LOWPOWER_OFF, kpd_input_dev->keybit);
#endif
	...
	init_proc_lowpower_key_state();

	mt_set_gpio_mode(GPIO_QWERTYLOWPOWER_EINT_PIN, GPIO_QWERTYLOWPOWER_EINT_PIN_M_EINT);
	mt_set_gpio_dir(GPIO_QWERTYLOWPOWER_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_QWERTYLOWPOWER_EINT_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_QWERTYLOWPOWER_EINT_PIN, GPIO_PULL_UP);

	mt65xx_eint_set_sens(KPD_LOWPOWER_EINT, KPD_LOWPOWER_SENSITIVE);
	mt65xx_eint_set_hw_debounce(KPD_LOWPOWER_EINT, KPD_LOWPOWER_DEBOUNCE);
	mt65xx_eint_registration(KPD_LOWPOWER_EINT, true, KPD_LOWPOWER_POLARITY,
                         kpd_lowpower_eint_handler, false);
#endif	
