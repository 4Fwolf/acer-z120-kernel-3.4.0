...
/*----------------------------------------------------------------------------*/
int kxtf9_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = KXTF9_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
...
static int kxtf9_resume(struct i2c_client *client)
{
	...
	if (0)//(err = kxtf9_init_client(client, 0))  //longxuewei
	{
...
static void kxtf9_late_resume(struct early_suspend *h)
{
	...
	KXTF9_power(obj->hw, 1);
	if (0)//(err = kxtf9_init_client(obj->client, 0))  //longxuewei
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int kxtf9_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif 
	kxtf9_init_flag = 0;/* Add for auto detect feature */
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
	...
	kfree(obj);
	exit:
	kxtf9_init_flag = -1;/* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);        
	return err;
}
