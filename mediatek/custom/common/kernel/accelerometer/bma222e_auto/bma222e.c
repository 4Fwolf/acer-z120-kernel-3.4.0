...
/*----------------------------------------------------------------------------*/
#define I2C_DRIVERID_BMA222E 224
/*----------------------------------------------------------------------------*/
#define DEBUG 1
/*----------------------------------------------------------------------------*/
...
static int bma222e_init_flag =0;
...
static void bma222e_late_resume(struct early_suspend *h)
{
	...
	BMA222E_power(obj->hw, 1);
	if (0)//(err = bma222e_init_client(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int bma222e_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = bma222e_get_cust_acc_hw();  /*for auto detect feature*/
	...
	if(err = bma222e_create_attr(&bma222e_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma222e_gsensor_operate;
	...
	bma222e_init_flag = 0;/* Add for auto detect feature */
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
	...
	kfree(obj);
	exit:
	bma222e_init_flag = -1;  /* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);   
	return err;
}
...
static int bma222e_i2c_remove(struct i2c_client *client)
{
	...
	if(err = bma222e_delete_attr(&bma222e_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
