	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = MMA8452Q_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
...
static int mma8452q_resume(struct i2c_client *client)
{
	...
	if(0)//((err = MMA8452Q_Init(client, 0)))
	{
...
static void mma8452q_late_resume(struct early_suspend *h)
{
	...
	MMA8452Q_power(obj->hw, 1);
	if(0)//((err = MMA8452Q_Init(obj->client, 0)))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
