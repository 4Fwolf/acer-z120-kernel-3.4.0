	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = KXTIK1004_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
...
static int kxtik1004_resume(struct i2c_client *client)
{
	...
	if (0)//(err = kxtik1004_init_client(client, 0))
	{
...
static void kxtik1004_late_resume(struct early_suspend *h)
{
	...
	KXTIK1004_power(obj->hw, 1);
	if (0)//(err = kxtik1004_init_client(obj->client, 0))
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int kxtik1004_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = kxtik1004_get_cust_acc_hw();  /*for auto detect feature*/
	...
	//if(err = kxtik1004_create_attr(&kxtik1004_gsensor_driver.driver))
        if(err = kxtik1004_create_attr(&kxtik1004_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
...
#endif 
        kxtik1004_init_flag = 0;/* Add for auto detect feature */
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
	...
	kfree(obj);
	exit:
        kxtik1004_init_flag = -1;  /* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);        
	return err;
}
...
static int kxtik1004_i2c_remove(struct i2c_client *client)
{
	...
	//if(err = kxtik1004_delete_attr(&kxtik1004_gsensor_driver.driver))
        if(err = kxtik1004_delete_attr(&kxtik1004_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
