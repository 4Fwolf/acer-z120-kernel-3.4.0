...
#include "s5k4e1garaw_Sensor.h"
#include "s5k4e1garaw_Camera_Sensor_para.h"
#include "s5k4e1garaw_CameraCustomized.h"

static DEFINE_SPINLOCK(s5k4e1garaw_drv_lock);
...
#define S5K4E1GA_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, S5K4E1GA_WRITE_ID)

kal_uint16 S5K4E1GA_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
    	iReadReg((u16) addr ,(u8*)&get_byte,S5K4E1GA_WRITE_ID);
   	 return get_byte;
}
...
/////////////////////////////////////////////////////////
//
//max_shutter = frame_length - vblank;
//    ==> vblank = frame_length - max_shutter
//
//for s5k4e1ga application note:
//
//    Integration = ( ( line_length_pck * coarse_integration_time ) + fine_integration_time ) / VT_pix_clk_freq_mhz
//
//    Min  [coarse_integration_time] : 3
//    Max [coarse_integration_time] : frame_length - 8
//
//    Min  [fine_integration_time]: 0x00
//    Max [fine_integration_time]: line_length_pck
//
//    framerate = pclk / ( frame_length * line_length ) * unit(mhz to s)
//
//    fix line_length
//
//	  insert dummy is  vblank
//
/////////////////////////////////////////////////////////
...
void S5K4E1GA_write_shutter(kal_uint16 shutter)
{
	...
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]write shutter = %d\n", shutter);
	#endif
	...
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]max_shutter = %d\n", max_shutter);
	SENSORDB("[S5K4E1]extra_lines = %d\n", extra_lines);
	SENSORDB("[S5K4E1]line_length = %d\n",line_length);
	SENSORDB("[S5K4E1]frame_length = %d\n",frame_length);
	SENSORDB("[S5K4E1]s5k4e1ga.pvPclk = %d\n",s5k4e1ga.pvPclk);
	SENSORDB("[S5K4E1]s5k4e1ga.capPclk = %d\n",s5k4e1ga.capPclk);
	SENSORDB("[S5K4E1]s5k4e1ga.DummyPixels = %d\n",s5k4e1ga.DummyPixels);
	SENSORDB("[S5K4E1]s5k4e1ga.DummyLines  = %d\n",s5k4e1ga.DummyLines );
	SENSORDB("[S5K4E1]framerate = now_framerate = %d\n",now_framerate);
	#endif
	...
	//S5K4E1GA_write_cmos_sensor(0x0105,0x01);//mask corrupted frame
	
	//Set total frame length
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]write frame_length = %d\n", frame_length);
	SENSORDB("[S5K4E1]write shutter = %d\n", shutter);
	#endif
	
	S5K4E1GA_write_cmos_sensor(0x0340, (frame_length >> 8) & 0xFF);
	S5K4E1GA_write_cmos_sensor(0x0341, frame_length & 0xFF);
	spin_lock_irqsave(&s5k4e1garaw_drv_lock,flags);
	s5k4e1ga.maxExposureLines = frame_length - 8; //4
	spin_unlock_irqrestore(&s5k4e1garaw_drv_lock,flags);
...
{
	spin_lock(&s5k4e1garaw_drv_lock);
	s5k4e1ga.sensorGlobalGain = gain;
	spin_unlock(&s5k4e1garaw_drv_lock);

	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]gain\n");
	SENSORDB("[S5K4E1]write gain = %d\n",gain);
	#endif
...
void S5K4E1GA_SetGain(UINT16 iGain)
{
	...
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]set gain\n");
	SENSORDB("[S5K4E1]isp gain = %d\n",iGain);
	#endif
	...
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]sensorBaseGain = %d\n",s5k4e1ga.sensorBaseGain);
	SENSORDB("[S5K4E1]ispBaseGain = %d\n",s5k4e1ga.ispBaseGain);
	SENSORDB("[S5K4E1] write gain is sensorGlobalGain = %d\n",s5k4e1ga.sensorGlobalGain);
	#endif
...
kal_uint16 read_S5K4E1GA_gain(void)
{
	...
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]read gain\n");
	SENSORDB("[S5K4E1]sensor_gaingain = %d\n",sensor_gain);
	#endif
...
static void S5K4E1GA_SetDummy( const kal_uint16 iPixels, const kal_uint16 iLines )
{
	...
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]set dummy");
	SENSORDB("[S5K4E1]write line_length = %d\n",line_length);
	SENSORDB("[S5K4E1]write frame_length = %d\n",frame_length);
	SENSORDB("[S5K4E1]maxExposureLines = %d\n",s5k4e1ga.maxExposureLines);
	#endif
...
{
#ifdef S5K4E1GA_DEBUG_OPEN
 SENSORDB("[S5K4E1]PreviewSetting enter\n ");
#endif
 ...
 //S5K4E1GA_write_cmos_sensor(0x3030, 0x06);	//RAW 10:0x06;RAW8::0x05
 S5K4E1GA_write_cmos_sensor(0x3112, 0x01);
 S5K4E1GA_write_cmos_sensor(0x3030, 0x06);
 ...
 S5K4E1GA_write_cmos_sensor(0x30BC,0xB0);// 0XB0/////////////////////////////
 ...
 S5K4E1GA_write_cmos_sensor(0x30BE,0x08);//M_PCLKDIV_AUTO[4], M_DIV_PCLK[3:0]
 S5K4E1GA_write_cmos_sensor(0x30E2,0x01);
 //
 S5K4E1GA_write_cmos_sensor(0x3110,0x00);//modify bit4 0--> 1 //Jack 20121221 10=>
 S5K4E1GA_write_cmos_sensor(0x3117,0x06);
S5K4E1GA_write_cmos_sensor(0x3119,0x0F);//Jack 20121221 0A=>
S5K4E1GA_write_cmos_sensor(0x311A,0x7A);//Jack 20121221 AA=>BA
	
 /*// Integration setting
 //980
 S5K4E1GA_write_cmos_sensor(0x0202, 0x03); //
 S5K4E1GA_write_cmos_sensor(0x0203, 0xD4); //A8
 //4X
 ...
 */
 ...
 // PLL setting ...
 //// input clock 26MHz
 ////// (3)  1-lane Serial(TST = 0000b or TST = 0010b), 15 fps
 S5K4E1GA_write_cmos_sensor(0x0304, 0x00); //P=4
 S5K4E1GA_write_cmos_sensor(0x0305, 0x04); //P=4
 S5K4E1GA_write_cmos_sensor(0x0306, 0x00); 
 S5K4E1GA_write_cmos_sensor(0x0307, 0x36); //M=50 ;0X36--648;0X66->816
 //reserved
 S5K4E1GA_write_cmos_sensor(0x30B5, 0x00);
 S5K4E1GA_write_cmos_sensor(0x30BE, 0x08); //DIV_M_PCLK s
 //24M/4*2*50/1/10=60M
...
S5K4E1GA_write_cmos_sensor(0x300E,0xAB);//Vertical Binning On
...
{
#ifdef S5K4E1GA_DEBUG_OPEN
SENSORDB("[S5K4E1]CaptureSetting enter\n ");
#endif
 S5K4E1GA_write_cmos_sensor(0x0100, 0x00);	// SW stand-by	 
...
// S5K4E1GA_write_cmos_sensor(0x3030, 0x06);	//RAW 10
 ...
 S5K4E1GA_write_cmos_sensor(0x30E2,0x01);
 //
 S5K4E1GA_write_cmos_sensor(0x3110,0x00);//modify 0x00 --> 0x10// maliyan 0x10->0x00
 S5K4E1GA_write_cmos_sensor(0x3117,0x06);
S5K4E1GA_write_cmos_sensor(0x3119,0x0F);//maliyan 0x0A->0x0F
S5K4E1GA_write_cmos_sensor(0x311A,0x7A);//maliyan 0xAA->0x7A
 /*// Integration setting
 // 1576
 ...
 */
 ...
 // PLL setting ...
 //// input clock 24MHz
 S5K4E1GA_write_cmos_sensor(0x0304, 0x00);
 S5K4E1GA_write_cmos_sensor(0x0305, 0x04);
 S5K4E1GA_write_cmos_sensor(0x0306, 0x00);
 S5K4E1GA_write_cmos_sensor(0x0307, 0x36);
 //reserved
 S5K4E1GA_write_cmos_sensor(0x30B5, 0x00);
 S5K4E1GA_write_cmos_sensor(0x30E2, 0x08);	
 ...
 */
 ...
 S5K4E1GA_write_cmos_sensor(0x0104, 0x00);
...
{
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]Sensor_Init enter\n ");
	#endif
...
UINT32 S5K4E1GAOpen(void)
{
	...
	for(i=0;i<3;i++)
	{
	   ...
	   SENSORDB("[S5K4E1]READ ID :%x",sensor_id);
...
UINT32 S5K4E1GAGetSensorID(UINT32 *sensorID) 
{
    ...
    printk("[S5K4E1GAGetSensorID]:Read Sensor ID :0x%x\n", *sensorID); 
...
void S5K4E1GA_SetShutter(kal_uint16 iShutter)
{
   ...
   s5k4e1ga.shutter= iShutter;
   	...
   	#ifdef S5K4E1GA_DEBUG_OPEN
   	SENSORDB("[S5K4E1]Set Shutter");
   	SENSORDB("[S5K4E1]shutter = %d\n", s5k4e1ga.shutter);
	SENSORDB("[S5K4E1]write shutter\n");
	#endif
...
void S5K4E1GASetFlipMirror(kal_int32 imgMirror)
{
    ...
    {
        case IMAGE_NORMAL: //B
            S5K4E1GA_write_cmos_sensor(0x0101, 0x00);	//Set normal
            break;
        ...
        case IMAGE_HV_MIRROR: //R
            S5K4E1GA_write_cmos_sensor(0x0101, 0x03);	//Set mirror and flip
            break;
    }
	
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]Flip Mirror=%d\n",imgMirror);
	#endif
}
...
{
	kal_uint32 PreviewShutterValue;
	
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]Preview enter:\n");
	#endif
	...
	PreviewShutterValue = S5K4E1GA_read_shutter();
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]PreviewShutterValue = %d:\n",PreviewShutterValue);
	#endif
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	kal_uint32 CaptureReadShutterValue;
	
	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]Capture enter:\n");
	#endif

	CaptureReadShutterValue = S5K4E1GA_read_shutter();

	#ifdef S5K4E1GA_DEBUG_OPEN
	SENSORDB("[S5K4E1]CaptureReadShutterValue = %d:\n",CaptureReadShutterValue);
	#endif
...
UINT32 S5K4E1GAGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	#else
    	pSensorResolution->SensorPreviewWidth	= (S5K4E1GA_IMAGE_SENSOR_FULL_WIDTH-4-8); //S5K4E1GAMIPI_REAL_PV_WIDTH;
    	pSensorResolution->SensorPreviewHeight	= (S5K4E1GA_IMAGE_SENSOR_FULL_HEIGHT-4); //S5K4E1GAMIPI_REAL_PV_HEIGHT;
    #endif
    pSensorResolution->SensorFullWidth		= (S5K4E1GA_IMAGE_SENSOR_FULL_WIDTH-4-8); //S5K4E1GAMIPI_REAL_CAP_WIDTH;
    pSensorResolution->SensorFullHeight		= (S5K4E1GA_IMAGE_SENSOR_FULL_HEIGHT-4); //S5K4E1GAMIPI_REAL_CAP_HEIGHT;
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	#else
		pSensorInfo->SensorPreviewResolutionX= (S5K4E1GA_IMAGE_SENSOR_FULL_WIDTH-4-8); //S5K4E1GA_IMAGE_SENSOR_FULL_WIDTH;
		pSensorInfo->SensorPreviewResolutionY=(S5K4E1GA_IMAGE_SENSOR_FULL_HEIGHT-4) ;//S5K4E1GAMIPI_REAL_CAP_HEIGHT;
		pSensorInfo->SensorCameraPreviewFrameRate=15;
	#endif
	pSensorInfo->SensorFullResolutionX= (S5K4E1GA_IMAGE_SENSOR_FULL_WIDTH-4-8);
   	 pSensorInfo->SensorFullResolutionY= (S5K4E1GA_IMAGE_SENSOR_FULL_HEIGHT-4);

	spin_lock(&s5k4e1garaw_drv_lock);
	s5k4e1ga.imgMirror = pSensorConfigData->SensorImageMirror ;
	spin_unlock(&s5k4e1garaw_drv_lock);
	SENSORDB("[S5K4E1GAGetInfo]SensorImageMirror:%d\n", s5k4e1ga.imgMirror );
	  printk("[S5K4E1]Getinfo\n");
    ...
    switch (ScenarioId)
    {
        ...
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_MEM:
            ...
            pSensorInfo->SensorPixelClockCount= 3;
            pSensorInfo->SensorDataLatchCount= 2;
            pSensorInfo->SensorGrabStartX = 4*S5K4E1GA_FULL_X_START;	//2*S5K4E1GA_IMAGE_SENSOR_PV_STARTX; 
            pSensorInfo->SensorGrabStartY = 2*S5K4E1GA_FULL_Y_START;	//2*S5K4E1GA_IMAGE_SENSOR_PV_STARTY;          			
            pSensorInfo->SensorWidthSampling = 0;  // 0 is default 1x
            pSensorInfo->SensorHightSampling = 0;   // 0 is default 1x
            pSensorInfo->SensorPacketECCOrder = 1;
            break;
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        case SENSOR_FEATURE_GET_RESOLUTION:
            *pFeatureReturnPara16++= (S5K4E1GA_IMAGE_SENSOR_FULL_WIDTH-4-8);
            *pFeatureReturnPara16= (S5K4E1GA_IMAGE_SENSOR_FULL_HEIGHT-4);
            *pFeatureParaLen=4;
            break;
...
UINT32 S5K4E1GA_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
