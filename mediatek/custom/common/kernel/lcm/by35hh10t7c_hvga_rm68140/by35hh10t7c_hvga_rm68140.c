...
{
#ifdef BUILD_LK
	printf("init_lcm_registers -----by35hh \r\n");
#else
	printk("init_lcm_registers-----by35hh  \r\n");
#endif
    //LK@Z1
    send_ctrl_cmd(0xC0);
    send_data_cmd(0x10);//0x12
    send_data_cmd(0x10);//0x12
    ...
    send_ctrl_cmd(0xC5);//set vcom
    send_data_cmd(0x00);
    send_data_cmd(0x30);
    send_data_cmd(0x80);

        
    send_ctrl_cmd(0xB0);//set interface mode
    send_data_cmd(0x82);//80
    ...
    send_ctrl_cmd(0xB4);//set invrsion
    send_data_cmd(0x02);
    
    send_ctrl_cmd(0xB6);
    send_data_cmd(0x32);//0x00
    send_data_cmd(0x22);//02
    send_data_cmd(0x3B);
...
/*
    ...
    //send_data_cmd(0x83);//bit7=7,sdo disable,sdo/sdi use the same pin
    send_data_cmd(0x03);//bit7=0,sdo en
    id1=spi_get_data(0xbf);
    id1=spi_get_data(0xbf);
    id1=spi_get_data(0xbf);
    id2=spi_get_data(0xbf);
    id = (id1<<8)|id2;//ic id
#ifdef BUILD_LK
...
// ---------------------------------------------------------------------------
LCM_DRIVER by35hh10t7c_hvga_rm68140_lcm_drv = 
{
    .name		= "by35hh10t7c_rm68140",
