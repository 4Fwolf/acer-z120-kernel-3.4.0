...
{
#ifdef BUILD_LK
	printf("init_lcm_registers -----by35hp \r\n");
#else
	printk("init_lcm_registers-----by35hp  \r\n");
#endif
    //LK@Z1
	...
	send_ctrl_cmd(0xB6); //
	send_data_cmd(0x50); //6f VCOMDC//Vcom 59->50
...
*/
	...
	send_data_cmd(0x1a); //1c VSPR 0x1E->0x1a,for contrast
	send_data_cmd(0x1a); //1c VSNR
	send_data_cmd(0x83); //83 AP  83
	send_data_cmd(0xAA); //FS

	send_ctrl_cmd(0xC0); //STBA
	send_data_cmd(0x36); //OPON 36
	send_data_cmd(0x36); //OPON 36
	send_data_cmd(0x01); //
	send_data_cmd(0x3C); //
	send_data_cmd(0xC8); //
...
/* gamma backup:
	...
	send_data_cmd(0x04); //06 2//
	send_data_cmd(0x09); //3
	send_data_cmd(0x13); //4
	send_data_cmd(0x28); //5
	send_data_cmd(0x3f); //6
	send_data_cmd(0x48); //7
	send_data_cmd(0x51); //8
	send_data_cmd(0x4a); //9
	send_data_cmd(0x42); //10
	send_data_cmd(0x3f); //11
	send_data_cmd(0x35); //12
	send_data_cmd(0x32); //13
	send_data_cmd(0x2c); //14
	send_data_cmd(0x2C); //15
	send_data_cmd(0x28); //16
...
// ---------------------------------------------------------------------------
LCM_DRIVER by35hp_hvga_hx8357_lcm_drv = 
{
    .name			= "by35hp_hx8357",
