 ...
 * 2012/11/1:  Release for MTK6577/6575/6573/6513 Platform
...
#include <mach/mt_boot.h> /*Set your boot */
...
//#define IAP_PORTION            	1//LK@2013-3-11,mask
#define PAGERETRY  30
#define IAPRESTART 5
...
//unsigned short force[] = {0, 0x10, I2C_CLIENT_END, I2C_CLIENT_END};
//static const unsigned short *const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces, };
static struct i2c_board_info __initdata elan_tpd_i2c_tpd={ I2C_BOARD_INFO(TPD_DEVICE, (0x2A>>1))};
...
static int __fw_packet_handler(struct i2c_client *client)
{
	...
	printk(KERN_INFO "[elan] %s: n", __func__);
// Firmware version
...
// for 2 fingers      
	...
	//printk("[elan_ktf2k_ts_report_data] buf[0]= %x num=%x\n",buf[0], num);
	switch (buf[0]) {
		case NORMAL_PKT:
		case TWO_FINGERS_PKT:
		case FIVE_FINGERS_PKT:	
		case TEN_FINGERS_PKT:
			     ...
			     if(button_state==0)
			   	{
					...
					{
            					//printk("tp buf[BUTTON_ID_INDEX] = %x ELAN_KEY_BACK=%x \n",buf[BUTTON_ID_INDEX],ELAN_KEY_BACK);
						button_state = ELAN_KEY_BACK;
						input_report_key(idev, KEY_BACK, 1);
					} 
					else if (buf[BUTTON_ID_INDEX] == ELAN_KEY_HOME) 
					{
            					//printk("tp buf[BUTTON_ID_INDEX] = %x ELAN_KEY_HOME=%x \n",buf[BUTTON_ID_INDEX],ELAN_KEY_HOME);
						button_state = ELAN_KEY_HOME;
						input_report_key(idev, KEY_HOMEPAGE, 1);
					} 
					else if (buf[BUTTON_ID_INDEX] == ELAN_KEY_MENU) 
					{
             					//printk("tp buf[BUTTON_ID_INDEX] = %x ELAN_KEY_MENU=%x \n",buf[BUTTON_ID_INDEX],ELAN_KEY_MENU);
			   	...
			   	{
					if (button_state== ELAN_KEY_BACK) 
					{
						input_report_key(idev, KEY_BACK, 0);
						button_state = 0;
					} 
					else if (button_state == ELAN_KEY_HOME) 
					{
						input_report_key(idev, KEY_HOMEPAGE, 0);
						button_state = 0;
					} 
					else if (button_state== ELAN_KEY_MENU) 
					{
			...
			{			
				...
				{	
					...
					{
						elan_ktf2k_ts_parse_xy(&buf[idx], &x, &y); 
						//printk("[elan_debug  BTN bit] %s, elan_x=%d, elan_y=%d\n",__func__, x , y);
						 x = ( x * LCM_X_MAX )/ELAN_X_MAX;
					        y = ( y * LCM_Y_MAX )/ELAN_Y_MAX;
						//printk("[elan_debug  BTN bit] %s, x=%d, y=%d\n",__func__, x , y);
						//x = X_RESOLUTION-x;	 
						//y = Y_RESOLUTION-y;			     
						//if (!((x<=0) || (y<=0) || (x>=LCM_X_MAX) || (y>=LCM_Y_MAX))) //LK@2013-3-11,mask
...
static int tpd_check_i2c(struct i2c_client *client)
{
    int retval = 0;
    uint8_t buf_recv[8] = { 0 };
    unsigned char i = 0;
    ...
    {
         retval = i2c_master_recv(client, buf_recv, 4);
         printk("[tpd_check_i2c] rx retval:%d;i=%d\n",retval,i);
         if(retval > 0)
         {
             printk("[tpd_check_i2c] rx retval:%d;i=%d,break\n",retval,i);
             break;
         }
         else
         {
             printk("[tpd_check_i2c] error: rx retval:%d;i=%d\n",retval,i);
         }
    }
...
// End Setup Interrupt Pin	
	...
	retval = tpd_check_i2c(client);
	if (retval < 0) {
           printk(KERN_INFO "[elan] tpd_check_i2c error:No Elan chip inside\n");
           tpd_load_status = 0;
           return -1;
	}
